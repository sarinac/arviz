(function() {
  const fn = function() {
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
    
    const element = document.getElementById("d60ae449-92e2-4390-bbdb-672b36947ebc");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid 'd60ae449-92e2-4390-bbdb-672b36947ebc' but no matching script tag was found.")
        }
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.3.min.js"];
      const css_urls = [];
    
      const inline_js = [    function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                  const docs_json = '{"d025bbfb-6333-4542-8b38-45f4a853624b":{"defs":[],"roots":{"references":[{"attributes":{"axis":{"id":"18065"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"18068","type":"Grid"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"18109"},"selection_policy":{"id":"18108"}},"id":"18088","type":"ColumnDataSource"},{"attributes":{"coordinates":null,"data_source":{"id":"18088"},"glyph":{"id":"18089"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18091"},"nonselection_glyph":{"id":"18090"},"view":{"id":"18093"}},"id":"18092","type":"GlyphRenderer"},{"attributes":{"source":{"id":"18088"}},"id":"18093","type":"CDSView"},{"attributes":{"source":{"id":"18116"}},"id":"18121","type":"CDSView"},{"attributes":{},"id":"18055","type":"DataRange1d"},{"attributes":{},"id":"18072","type":"WheelZoomTool"},{"attributes":{"coordinates":null,"data_source":{"id":"18116"},"glyph":{"id":"18117"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18119"},"nonselection_glyph":{"id":"18118"},"view":{"id":"18121"}},"id":"18120","type":"GlyphRenderer"},{"attributes":{},"id":"18074","type":"UndoTool"},{"attributes":{},"id":"18057","type":"LinearScale"},{"attributes":{},"id":"18104","type":"AllLabels"},{"attributes":{"data":{},"selected":{"id":"18136"},"selection_policy":{"id":"18135"}},"id":"18116","type":"ColumnDataSource"},{"attributes":{"toolbar":{"id":"18141"},"toolbar_location":"above"},"id":"18142","type":"ToolbarBox"},{"attributes":{},"id":"18110","type":"UnionRenderers"},{"attributes":{"fill_alpha":{"value":0.2},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.2},"line_alpha":{"value":0.2},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"18119","type":"Circle"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"18098"}]},"id":"18115","type":"LegendItem"},{"attributes":{},"id":"18069","type":"ResetTool"},{"attributes":{"children":[{"id":"18142"},{"id":"18140"}]},"id":"18143","type":"Column"},{"attributes":{},"id":"18108","type":"UnionRenderers"},{"attributes":{"overlay":{"id":"18077"}},"id":"18071","type":"BoxZoomTool"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18095","type":"Line"},{"attributes":{},"id":"18075","type":"SaveTool"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18090","type":"Line"},{"attributes":{"coordinates":null,"data_source":{"id":"18094"},"glyph":{"id":"18095"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18097"},"nonselection_glyph":{"id":"18096"},"view":{"id":"18099"}},"id":"18098","type":"GlyphRenderer"},{"attributes":{"coordinates":null,"formatter":{"id":"18106"},"group":null,"major_label_policy":{"id":"18107"},"ticker":{"id":"18062"}},"id":"18061","type":"LinearAxis"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"18078","type":"PolyAnnotation"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.1},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"18118","type":"Circle"},{"attributes":{"coordinates":null,"formatter":{"id":"18103"},"group":null,"major_label_policy":{"id":"18104"},"ticker":{"id":"18066"}},"id":"18065","type":"LinearAxis"},{"attributes":{},"id":"18059","type":"LinearScale"},{"attributes":{},"id":"18066","type":"BasicTicker"},{"attributes":{"data":{},"selected":{"id":"18111"},"selection_policy":{"id":"18110"}},"id":"18094","type":"ColumnDataSource"},{"attributes":{},"id":"18062","type":"BasicTicker"},{"attributes":{},"id":"18053","type":"DataRange1d"},{"attributes":{},"id":"18109","type":"Selection"},{"attributes":{},"id":"18135","type":"UnionRenderers"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18089","type":"Line"},{"attributes":{},"id":"18106","type":"BasicTickFormatter"},{"attributes":{"callback":null},"id":"18076","type":"HoverTool"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18091","type":"Line"},{"attributes":{"overlay":{"id":"18078"}},"id":"18073","type":"LassoSelectTool"},{"attributes":{"axis":{"id":"18061"},"coordinates":null,"group":null,"ticker":null},"id":"18064","type":"Grid"},{"attributes":{"source":{"id":"18094"}},"id":"18099","type":"CDSView"},{"attributes":{"below":[{"id":"18061"}],"center":[{"id":"18064"},{"id":"18068"},{"id":"18114"}],"height":500,"left":[{"id":"18065"}],"output_backend":"webgl","renderers":[{"id":"18092"},{"id":"18098"},{"id":"18120"}],"title":{"id":"18122"},"toolbar":{"id":"18079"},"toolbar_location":null,"width":500,"x_range":{"id":"18053"},"x_scale":{"id":"18057"},"y_range":{"id":"18055"},"y_scale":{"id":"18059"}},"id":"18052","subtype":"Figure","type":"Plot"},{"attributes":{"toolbars":[{"id":"18079"}],"tools":[{"id":"18069"},{"id":"18070"},{"id":"18071"},{"id":"18072"},{"id":"18073"},{"id":"18074"},{"id":"18075"},{"id":"18076"}]},"id":"18141","type":"ProxyToolbar"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"18122","type":"Title"},{"attributes":{},"id":"18103","type":"BasicTickFormatter"},{"attributes":{},"id":"18111","type":"Selection"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18096","type":"Line"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"18077","type":"BoxAnnotation"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"18117","type":"Circle"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18097","type":"Line"},{"attributes":{},"id":"18107","type":"AllLabels"},{"attributes":{},"id":"18136","type":"Selection"},{"attributes":{"coordinates":null,"group":null,"items":[{"id":"18115"}]},"id":"18114","type":"Legend"},{"attributes":{"tools":[{"id":"18069"},{"id":"18070"},{"id":"18071"},{"id":"18072"},{"id":"18073"},{"id":"18074"},{"id":"18075"},{"id":"18076"}]},"id":"18079","type":"Toolbar"},{"attributes":{},"id":"18070","type":"PanTool"},{"attributes":{"children":[[{"id":"18052"},0,0]]},"id":"18140","type":"GridBox"}],"root_ids":["18143"]},"title":"Bokeh Application","version":"2.4.3"}}';
                  const render_items = [{"docid":"d025bbfb-6333-4542-8b38-45f4a853624b","root_ids":["18143"],"roots":{"18143":"d60ae449-92e2-4390-bbdb-672b36947ebc"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
    function(Bokeh) {
        }
      ];
    
      function run_inline_js() {
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();